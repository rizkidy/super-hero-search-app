...
const hideAllTabHead= () => allTabsHead.forEach(singelTabHead => singelTabHead.classList.remove('active-tab'));

// Event Listeners
window.addEventListener('DOMContentLoaded', () => init());

// Button event listerners
allTabsHead.forEach(singelTabHead => {
    singelTabHead.addEventListener('click', () => {
        hideAllTabHead();
        activeTab = singelTabHead.dataset.id;
        showActiveTabBody();
        showActiveTabHead();
    })
})
...
// API Key => 3681260272102039
const fetchAllSuperHero = async(searchText) => {
    let url = `https://www.superheroapi.com/api.php/3681260272102039/search/${searchText}`;
    try {
